...
contract Mint is ReentrancyGuard, Pausable {
  ...
  constructor(address _role, uint count) {
    roleAdd =_role;
    totalNfts = count;
  }
  ...
  mapping(uint => bool) nftIsClaimed;
  ...
  function setNftSupply(uint _amount) public hasDevAdmin returns(bool){
    require(totalNfts.add(_amount) > totalNfts);
    totalNfts = totalNfts.add(_amount);
    return true;
  }
  ...
  /// @notice
  /*~~~>
    Fuctions for fetching a random tokenId from unclaimed NFTs
  <~~~*/
    function getRandom(uint nonce) internal returns(uint ramndomNumber){
        uint rambo = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, nonce))) % totalNfts;
        bool checking = true;
        while(checking){
